...
export default function Courses() {
  ...
  const [openModuleId, setOpenModuleId] = useState<string | null>(() => getCourseData().modules[0]?.id ?? null);
  const [openFaqId, setOpenFaqId] = useState<string | null>(null);

  useEffect(() => {
    const sync = () => setData(getCourseData());
  ...
  return (
    <>
      <main style={{ background: 'var(--navy)', minHeight: '100vh', paddingTop: '80px' }}>
        ...
        <section
          style={{
            background: 'linear-gradient(180deg, rgba(0,212,255,.05) 0%, transparent 60%)',
          }}
        >
          <div className="max-w-6xl mx-auto px-6 py-16 lg:py-24 grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-10 lg:gap-16 items-start">
            ...
            <div>
              ...
              >
                <span
                  ...
                  style={{ background: 'var(--electric)', display: 'inline-block' }}
                />
                {data.badge}
              </div>
              <h1
              ...
              <p
                className="text-lg leading-relaxed max-w-2xl mb-7 anim-3"
              ...
              <div className="flex flex-wrap gap-4 anim-5">
                <button type="button" onClick={modal.open} className="btn-primary px-8 py-3.5 text-base">
                  {data.cta1Text}
                </button>
                <button
                  type="button"
            ...
            {/* Right — sticky card */}
            <div
              className="glass-card rounded-2xl p-7 anim-4"
              style={{
            ...
            >
              <h3
                className="font-bold text-sm mb-5 pb-4"
                style={{ fontFamily: 'var(--font-head)', color: 'var(--white)', borderBottom: '1px solid var(--border)' }}
              >
                Course Highlights
              </h3>
                ...
                {data.cardHighlights.map((row) => (
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{ color: row.highlighted ? 'var(--electric)' : 'var(--white)' }}
                    >
                      {row.value}
                    </span>
                  </div>
              ...
              <button
                type="button"
                onClick={modal.open}
                className="btn-primary w-full py-3 mt-5 justify-center"
              >
                Book Free Demo →
              </button>
        ...
        {data.showAudience && (
          <>
            <section className="max-w-6xl mx-auto px-6 py-16 reveal">
              ...
              <h2 className="text-3xl md:text-4xl font-bold mb-3" style={{ fontFamily: 'var(--font-head)', color: 'var(--white)', letterSpacing: '-0.5px' }}>
                {data.audienceTitle}
              </h2>
              <p className="text-base mb-10 max-w-2xl" style={{ color: 'var(--muted)', lineHeight: 1.75 }}>
                {data.audienceSubtext}
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {data.audience.map((card) => (
                  <div
                    key={card.id}
                    className="glass-card glass-card-hover flex items-start gap-4 p-5 rounded-2xl"
                  >
                    <div
                      className="flex-shrink-0 w-11 h-11 rounded-xl flex items-center justify-center text-xl"
                      style={{ background: 'rgba(0,212,255,.08)' }}
                    >
                      {card.emoji}
                    </div>
                    <div>
                      <h4 className="font-bold text-sm mb-1" style={{ fontFamily: 'var(--font-head)', color: 'var(--white)' }}>
                        {card.title}
                      </h4>
                  ...
                  return (
                    <div key={mod.id} style={{ borderTop: idx === 0 ? 'none' : '1px solid var(--border)' }}>
                      <button
                        ...
                        style={{
                          background: isOpen ? 'rgba(0,212,255,.04)' : 'transparent',
                          borderLeft: isOpen ? '3px solid var(--electric)' : '3px solid transparent',
                        }}
                      >
                        <span
                          className="flex-shrink-0 text-xs font-bold tracking-widest uppercase min-w-[80px]"
            ...
            <section className="max-w-6xl mx-auto px-6 py-16 reveal">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {data.tools.map((tool) => (
                  <div
                    key={tool.id}
                    className="glass-card glass-card-hover p-5 rounded-2xl flex flex-col items-center text-center"
                  >
              ...
              {data.toolsMoreText && (
                <p className="text-center mt-6 text-sm" style={{ color: 'var(--muted)' }}>
                  {data.toolsMoreText.split('many more').length > 1 ? (
                    <>
            ...
            <section className="max-w-6xl mx-auto px-6 py-16 reveal">
              ...
              {data.showBeforeAfter && (
                <div className="mt-12 grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-6 items-center">
                  ...
                  <div
                    className="glass-card p-7 rounded-2xl"
                    style={{ border: '1px solid rgba(0,212,255,.2)', background: 'rgba(0,212,255,.04)' }}
                  >
                    <p className="text-xs font-bold tracking-widest uppercase mb-4" style={{ color: 'var(--electric)' }}>
                      {data.afterLabel}
                    </p>
                    {data.afterItems.map((item, i) => (
                      <div key={i} className="flex items-center gap-3 mb-3 last:mb-0 text-sm" style={{ color: 'var(--white)' }}>
                        <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: 'var(--electric)' }} />
                        {item}
                      </div>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {data.testimonials.map((t) => (
                  <div key={t.id} className="glass-card glass-card-hover p-6 rounded-2xl">
                    ...
                    <p className="text-sm leading-relaxed italic mb-4" style={{ color: 'var(--muted)' }}>
                      "{t.quote}"
                    </p>
        ...
        {data.showFaq && (
          <>
            <section className="max-w-3xl mx-auto px-6 py-16 reveal">
              <STag>FAQ</STag>
              <h2 className="text-3xl md:text-4xl font-bold mb-3 text-center" style={{ fontFamily: 'var(--font-head)', color: 'var(--white)', letterSpacing: '-0.5px' }}>
                {data.faqTitle}
              </h2>
              <p className="text-base mb-10 text-center" style={{ color: 'var(--muted)', lineHeight: 1.75 }}>
                {data.faqSubtext}
              </p>
                ...
                {data.faqs.map((faq) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ borderColor: isOpen ? 'rgba(0,212,255,.25)' : undefined }}
                    >
                      <button
                        type="button"
                      ...
                      >
                        ...
                        <span
                          className="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs border transition-all"
                          style={{
                            background: isOpen ? 'rgba(0,212,255,.1)' : 'transparent',
                            borderColor: isOpen ? 'var(--electric)' : 'var(--border)',
          ...
          <>
            <section className="max-w-6xl mx-auto px-6 py-16 reveal">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <NavLink
                  to="/courses"
                  className="glass-card glass-card-hover p-8 rounded-2xl no-underline block"
                  style={{ borderTop: '2.5px solid var(--orange)' }}
                >
                  <div className="text-xs font-bold tracking-widest uppercase mb-2" style={{ color: 'var(--orange)' }}>
                    Level 2A – Non-Tech Track
                  </div>
                  <div className="text-2xl font-bold mb-2" style={{ fontFamily: 'var(--font-head)', color: 'var(--white)' }}>
                    AI Generalist Program
                  </div>
                  <p className="text-sm leading-relaxed mb-5" style={{ color: 'var(--muted)' }}>
                    For freshers, professionals, and entrepreneurs. Master 15+ AI tools for content, design, video, automation, and productivity.
                  </p>
                ...
                <NavLink
                  to="/courses"
                  className="glass-card glass-card-hover p-8 rounded-2xl no-underline block"
                  style={{ borderTop: '2.5px solid #a78bfa' }}
                >
                  <div className="text-xs font-bold tracking-widest uppercase mb-2" style={{ color: '#a78bfa' }}>
                    Level 2B – Tech Track
                  </div>
                  <div className="text-2xl font-bold mb-2" style={{ fontFamily: 'var(--font-head)', color: 'var(--white)' }}>
                  ...
                  <p className="text-sm leading-relaxed mb-5" style={{ color: 'var(--muted)' }}>
                    For IT and engineering students. Learn AI-assisted coding, LLM APIs, GitHub Copilot, and build your own AI-powered applications.
                  </p>
        ...
        <section className="max-w-6xl mx-auto px-6 py-24 text-center relative reveal">
          <div
            ...
            style={{ background: 'radial-gradient(ellipse at center, rgba(0,212,255,.05) 0%, transparent 70%)' }}
          />
          <STag>Start Today</STag>
          <h2
            className="relative text-4xl md:text-6xl font-bold mb-5"
            style={{ fontFamily: 'var(--font-head)', letterSpacing: '-1.5px', color: 'var(--white)', lineHeight: 1.1, whiteSpace: 'pre-line' }}
          >
            {data.finalCtaTitle}
          </h2>
          <p className="relative text-lg max-w-xl mx-auto mb-10" style={{ color: 'var(--muted)', lineHeight: 1.7 }}>
            {data.finalCtaBody}
          </p>
          ...
          {data.finalCtaNote && (
            <p className="relative mt-5 text-sm" style={{ color: 'var(--muted)' }}>
              {data.finalCtaNote}
            </p>
          )}
        </section>
